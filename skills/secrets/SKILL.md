---
name: secrets
description: Scan for hardcoded secrets, API keys, passwords, and private keys (including git history guidance) and produce a containment and rotation plan. Use when checking for leaked credentials or secret exposure.
license: MIT
...
Tip: this bundle includes a runnable helper template: `../../plugins/crypto/tools/secrets-quickscan.sh`.
...
- `detect-secrets` for pre-commit/CI prevention (see `../../plugins/crypto/reference/tooling/detect-secrets.md`)
    ...
    for fn in files:
        ...
        for m in pat.finditer(text):
            ...
            if e >= 4.0:
                ...
                print(f"{path}:{text.count(chr(10), 0, m.start())+1}: entropy={e:.2f} len={len(s)} sample={s[:4]}...{s[-4:]}")
PY
```
