---
name: tls
description: Harden TLS configurations (TLS 1.3 + safe TLS 1.2), including hybrid post-quantum key exchange rollout patterns where supported. Use when configuring or auditing TLS settings.
license: MIT
...
- nginx: `../../plugins/crypto/reference/implementations/tls/nginx-hardening.md`
- Apache httpd: `../../plugins/crypto/reference/implementations/tls/apache-hardening.md`
- Node.js: `../../plugins/crypto/reference/implementations/tls/nodejs-tls-config.md`
- Python: `../../plugins/crypto/reference/implementations/tls/python-ssl-config.md`
- Go: `../../plugins/crypto/reference/implementations/tls/go-tls-config.md`
- Java: `../../plugins/crypto/reference/implementations/tls/java-tls-config.md`
- .NET: `../../plugins/crypto/reference/implementations/tls/dotnet-tls-config.md`
...
See:
- `../../plugins/crypto/reference/pqc/support-matrix.md`
- `../../plugins/crypto/tools/pqc-handshake-matrix.sh` (helper template for OpenSSL group probing)
...
Dynamic:
- (quick) run the helper template `../../plugins/crypto/tools/tls-probe.sh` (create it in-repo via `Edit`)
- run `testssl.sh` or `sslyze` for endpoints you can reach
- use `openssl s_client` to confirm protocol and group negotiation
- add automated integration tests (see `../../plugins/crypto/reference/testing/tls.md`)
