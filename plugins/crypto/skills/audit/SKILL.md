---
name: audit
description: Produce a risk-ranked cryptography audit across a repo (classical + PQC readiness), including secrets exposure checks, dependency posture, and explicit coverage/confidence limits.
...
- `reference/audit/monorepos.md` (if the repo is large or polyglot)
- `reference/risk-scoring.md`
...
Do Level 1 plus:
- (optional) run `tools/bootstrap.sh` to make the helper scripts runnable and to see tool hints
- (optional) run the bundled helper script template to chain scanners (see `reference/audit/methodology.md`): `tools/crypto-audit-escalate.sh`
...
- generate an SBOM and scan it (see `reference/operations/supply-chain.md`)
...
- use the script template `tools/tls-probe.sh` for a quick protocol-floor check
